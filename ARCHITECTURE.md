...
- Path canonicality: commits must carry exactly-normalized vault paths (`normalizeVaultPath(p) === p`); the server rejects anything else, so its index keys and every client's normalized view can never diverge on `a\b.md`-style spellings.
