...
- Transport: HTTPS/WSS everywhere; clients refuse cleartext `http(s)/ws` for anything but localhost. Worker sees plaintext — it lives in the user's own Cloudflare account; threat model excludes the account owner and treats Cloudflare as the host (same trust class as Obsidian Sync's relay, minus the vendor).
- Device tokens: 256-bit random, stored hashed (SHA-256) in DO; revocation is server-side instant. Tokens never travel in URLs — WS auth rides the `hello` frame (the worker's `?token=` upgrade pre-auth remains only for pre-0.1.4 clients). Pairing codes: one-time, 10-min TTL, hashed, admin-issued only, drawn with rejection sampling (uniform characters).
- Token rotation: a token older than 90 days is re-issued in the `helloAck` (`nextToken`) — clients durably persist the replacement via the `onTokenRotated` hook (plugin, daemon, CLI) before anything else. The previous token survives a 24 h grace window, and a hello within the grace re-issues the replacement again (the client missed the earlier hand-off; the DO stores only hashes, so a fresh mint is the only way to hand it a working credential) — rotation can never wedge an honest device, and a leaked token has a bounded life once the device syncs again.
...
- Path case-safety: plain case-only renames are SAFE — the scan correlates them by hash into a single rename op, and `applyPull` orders any decomposed case-colliding delete+add pull pair delete-first, so a case-insensitive filesystem (Windows/macOS) never destroys the just-written file. Two files differing only by name case (or by Unicode normalization form — NFC vs NFD `é`) can no longer be CREATED: the server's admission gate (`pathSafetyViolation`, shared by the DO and the in-memory server) refuses any commit that would create a NEW live path whose **fold key** — NFC canonical form, lowercased (`foldKeyForPath`) — is already occupied by another live entry (`PATH_COLLIDES`, its own error code so clients drop the push into `skippedPaths` diagnostics instead of wedging the cycle; deliberately NOT a protocol violation, so honest retries are never disconnected). A case-only rename is exempt by construction (its source row frees its own key), and within one client plan a fold-paired delete+add orders the delete first (`resolve.ts`). Legacy pairs synced before the gate stay syncable — edits to existing heads flow; the gate only stops NEW colliders. When a case-insensitive client meets a legacy pair, its filesystem shows only one of the two files; the scan never pushes a deletion for the invisible twin and surfaces it via `SyncClientStatus.caseCollisions` (same channel as Windows-unsafe names), a `warn` log per cycle, and the plugin support bundle. The collision itself is not arbitrated — rename one of the pair to resolve it.
- Path canonicality: vault paths are NFC by definition — `normalizeVaultPath` canonicalizes every segment, and the client's scan canonicalizes listed disk names the same way (reads use the raw disk form; hashes and index keys use the canonical one), so macOS's NFD and everyone else's NFC are ONE synchronized path instead of forking twins. Commits must carry exactly-canonical vault paths (`normalizeVaultPath(p) === p`); the server rejects anything else, so its index keys and every client's normalized view can never diverge on `a\b.md`-style spellings or normalization forms.
- Admin: argon2 passphrase hash in DO (min 8 chars); signed session cookies — random session id + HMAC over the expiry — with a revocation floor (`meta.sessions_not_before`): `logout` and `passphrase-change` kill every outstanding admin session instantly, no server-side session state required.
- WS abuse bound: a socket is disconnected after 3 protocol violations (pre-auth sockets included). Device, vault, and snapshot display names are length-capped at the ingest boundary.
- Blob uploads carry an ownership tag (R2 custom metadata); a rejected upload — hash mismatch or oversize — can only evict the object its own request wrote, so a racing verified upload is never destroyed by an impostor's cleanup.
- `.obsidian/plugins/*/data.json` never syncs — plugin credentials (the plugin's own device token included) stay machine-local even when `.obsidian/` sync is opted in.
...
| 15 | Large attachments | Single PUT, hard 100 MB cap (enforced while streaming); chunked/resumable uploads deferred | v1's 8 MiB multipart-lease design now — no user has hit the cap; the lease blueprint remains the ready plan when one does |
