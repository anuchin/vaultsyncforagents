...
describe('happy path', () => {
  it('changes the passphrase: old one fails login, new one works, fresh cookie issued', async () => {
    await claim({ passphrase: 'old-one', vaultName: 'v' });
    const cookie = await adminLogin('old-one');

    const res = await change(cookie, 'old-one', 'new-one');
    ...
    expect((await post('/admin/login', { passphrase: 'old-one' })).status).toBe(401);
    expect((await post('/admin/login', { passphrase: 'new-one' })).status).toBe(200);
...
describe('auth + validation', () => {
  it('requires a valid admin session cookie', async () => {
    ...
    expect((await post('/admin/passphrase-change', { current: 'pppp', next: 'qqqq' })).status).toBe(401);
    expect((await change('vsa_admin=1.deadbeef', 'pppp', 'qqqq')).status).toBe(401);
  });
  ...
  it('validates next like claim does (string, min 4 chars) without burning the budget', async () => {
    await claim({ passphrase: 'pppp', vaultName: 'v' });
    const cookie = await adminLogin('pppp');
    for (const body of [
      { current: 'pppp', next: 'abc' },
      { current: 'pppp', next: '' },
      { current: 'pppp', next: 42 },
      { current: 'pppp' },
      { next: 'qqqq' },
      {},
    ]) {
    ...
    expect((await post('/admin/login', { passphrase: 'pppp' }, IP)).status).toBe(200);
  });
...
describe('session rotation', () => {
  it('kills EVERY pre-existing admin cookie, not just the acting one', async () => {
    await claim({ passphrase: 'a-pass', vaultName: 'v' });
    const tab1 = await adminLogin('a-pass');
    const tab2 = await adminLogin('a-pass');

    const res = await change(tab2, 'a-pass', 'b-pass');
    expect(res.status).toBe(200);
...
describe('devices are untouched by a rotation', () => {
  it('paired devices keep syncing: Bearer status + WS hello still succeed', async () => {
    const claimed = await claim({ passphrase: 'old-one', deviceName: 'Laptop' });
    const cookie = await adminLogin('old-one');
    ...
    expect((await change(cookie, 'old-one', 'new-one')).status).toBe(200);
...
describe('throttling (shared per-IP budget with /admin/login and /pair)', () => {
  ...
  it('a successful change clears the budget (same semantics as login)', async () => {
    ...
    expect((await change(cookie, 'right-one', 'new-one', IP)).status).toBe(200);
...
describe('CORS: same-origin only (admin surface)', () => {
  it('responses carry no Access-Control-* headers, success and error alike', async () => {
    ...
    const ok = await change(cookie, 'pppp', 'qqqq', { origin: PLUGIN_ORIGIN });
    expect(ok.status).toBe(200);
    // Wrong current (with the fresh cookie — the old one just died).
    const bad = await change(cookieOf(ok), 'nope', 'rrrr', { origin: PLUGIN_ORIGIN });
