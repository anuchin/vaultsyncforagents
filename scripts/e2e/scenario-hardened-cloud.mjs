...
try {
  ...
  // ---- S2: X/Y PROGRESS — 30-note burst in vault4, capture progress, assert arrival in vault5 ----
  {
    ...
    const runOnce = async () => {
      ...
      // all 30 must arrive in vault5 BYTE-IDENTICAL (existence alone is not
      // arrival: on re-runs the files pre-exist with older content)
      const expectedJson = jstr(Object.fromEntries(made.value));
      const arrival = await waitFor(async () => {
        const missing = await cdp5.eval(`(async () => {
          ...
          for (const [name, text] of Object.entries(expected)) {
            let t = null;
            try { t = await app.vault.adapter.read(name); } catch (e) { /* absent */ }
            if (t !== text) missing.push(name);
          }
          return missing; })()`);
        const v = r_ok(missing);
        return Array.isArray(v) && v.length === 0 ? true : null;
      }, 45_000, 500, '30 burst notes arrive byte-identical in vault5');
  ...
  // ---- S3: PAUSE/RESUME — vault5 paused must not receive an edit; resume catches up ----
  {
    ...
    const runOnce = async () => {
      // defensive: a previous failed attempt may have left vault5 paused —
      // resume first so the seed note can arrive (retry-safety).
      await cdp5.eval(`(async () => { const p = app.plugins.plugins.vaultsyncforagents; if (p.syncingPaused) await p.resumeSyncing(); return 'ok'; })()`);
      ...
      if (paused.value.paused !== true || !String(paused.value.statusBar ?? '').includes('⏸')) {
        throw new Error(`pause state wrong: ${fmt(paused.value)}`);
      }
      log(`  vault5 paused: statusBar=${jstr(paused.value.statusBar)} hasClient=${paused.value.hasClient} state=${paused.value.state}`);
