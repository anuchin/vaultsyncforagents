...
try {
  ...
  const folderOps = [
    {
      id: 'F1',
      name: 'folder create with content: projects/a.md + projects/b.md in vault4 → vault5 gets both',
      async run() {
    ...
    },
    {
      ...
      async run(s) {
        ...
        const del = await cdp5.eval(`app.vault.delete(app.vault.getAbstractFileByPath('to-delete')).then(() => 'folder-deleted')`);
        ...
        if (propagated) {
          s.pass({ placeholderSyncMs: placeholderMs, deletePropagation: 'propagated within 40s', result: 'empty-folder deletion DID propagate' });
        } else {
          await syncNow(cdp4);
          await sleep(5000);
